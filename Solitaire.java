package solitaire;

import java.io.IOException;
import java.util.Scanner;
import java.util.Random;
import java.util.NoSuchElementException;

/**
 * This class implements a simplified version of Bruce Schneier's Solitaire Encryption algorithm.
 * 
 * @author RU NB CS112
 */
public class Solitaire {
	
	/**
	 * Circular linked list that is the deck of cards for encryption
	 */
	CardNode deckRear;
	
	/**
	 * Makes a shuffled deck of cards for encryption. The deck is stored in a circular
	 * linked list, whose last node is pointed to by the field deckRear
	 */
	public void makeDeck() {
		// start with an array of 1..28 for easy shuffling
		int[] cardValues = new int[28];
		// assign values from 1 to 28
		for (int i=0; i < cardValues.length; i++) {
			cardValues[i] = i+1;
		}
		
		// shuffle the cards
		Random randgen = new Random();
 	        for (int i = 0; i < cardValues.length; i++) {
	            int other = randgen.nextInt(28);
	            int temp = cardValues[i];
	            cardValues[i] = cardValues[other];
	            cardValues[other] = temp;
	        }
	     
	    // create a circular linked list from this deck and make deckRear point to its last node
	    CardNode cn = new CardNode();
	    cn.cardValue = cardValues[0];
	    cn.next = cn;
	    deckRear = cn;
	    for (int i=1; i < cardValues.length; i++) {
	    	cn = new CardNode();
	    	cn.cardValue = cardValues[i];
	    	cn.next = deckRear.next;
	    	deckRear.next = cn;
	    	deckRear = cn;
	    }
	}
	
	/**
	 * Makes a circular linked list deck out of values read from scanner.
	 */
	public void makeDeck(Scanner scanner) 
	throws IOException {
		CardNode cn = null;
		if (scanner.hasNextInt()) {
			cn = new CardNode();
		    cn.cardValue = scanner.nextInt();
		    cn.next = cn;
		    deckRear = cn;
		}
		while (scanner.hasNextInt()) {
			cn = new CardNode();
	    	cn.cardValue = scanner.nextInt();
	    	cn.next = deckRear.next;
	    	deckRear.next = cn;
	    	deckRear = cn;
		}
	}
	
	/**
	 * Implements Step 1 - Joker A - on the deck.
	 */
	void jokerA() {
		CardNode ptr = deckRear;
		while (true){
			if (ptr.next.cardValue == 27){
				if (ptr.next == deckRear){
					CardNode temp;
					temp = ptr.next.next;
					ptr.next.next = temp.next;
					temp.next = ptr.next;
					ptr.next = temp;
					deckRear = ptr.next;
					return;
				}
				else if (ptr.next.next == deckRear){
					CardNode temp;
					temp = deckRear;
					ptr.next.next = deckRear.next;
					temp.next = ptr.next;
					ptr.next = temp;
					deckRear = ptr.next.next;
					return;
				}
				else {
					CardNode temp;
					temp = ptr.next.next;
					ptr.next.next = temp.next;
					temp.next = ptr.next;
					ptr.next = temp;
					return;
				}
			}
			else {
				ptr = ptr.next;
			}
		}
	}
	
	/**
	 * Implements Step 2 - Joker B - on the deck.
	 */
	void jokerB() {
	    CardNode ptr = deckRear;
	    while (true){
	    	if (ptr.next.cardValue == 28){
	    		if (ptr.next == deckRear){
	    			CardNode temp = ptr.next.next;
	    			ptr.next.next = temp.next.next;
	    			temp.next.next = ptr.next;
	    			ptr.next = temp;
	    			deckRear = ptr.next;
	    			return;
	    		}
	    		else if (ptr.next.next == deckRear){
	    			CardNode temp = ptr.next.next;
	    			ptr.next.next = temp.next.next;
	    			temp.next.next = ptr.next;
	    			ptr.next = temp;
	    			deckRear = ptr.next.next;
	    			return;
	    		}
	    		else if (ptr.next.next.next == deckRear){
	    			CardNode temp = ptr.next.next;
	    			ptr.next.next = temp.next.next;
	    			temp.next.next = ptr.next;
	    			ptr.next = temp;
	    			deckRear = ptr.next.next.next;
	    			return;
	    		}
	    		else {
	    			CardNode temp = ptr.next.next;
	    			ptr.next.next = temp.next.next;
	    			temp.next.next = ptr.next;
	    			ptr.next = temp;
	    			return;
	    		}
	    	}
	    	else {
	    		ptr = ptr.next;
	    	}
	    }
	    
	}
	
	/**
	 * Implements Step 3 - Triple Cut - on the deck.
	 */
	void tripleCut() {
		boolean j1Assigned = false;
		CardNode j1 = null;
		CardNode j2 = null;
		CardNode ptr = deckRear;
		while (true){
			if ((ptr.next.cardValue == 27 || ptr.next.cardValue == 28) && !j1Assigned ){
				j1 = ptr;
				j1Assigned = true;
				ptr = ptr.next;
				continue;
			}
			else if (ptr.next.cardValue >= 27 && j1Assigned){
				j2 = ptr;
				break;
			}
			else {
				ptr = ptr.next;
			}
		}
		if (j1 == deckRear){
			
			deckRear = j2.next;
			return;
		}
		else if (j2.next == deckRear){
			deckRear = j1;
		}
		else {
			CardNode temp = j2.next.next;
			j2.next.next = deckRear.next;
			deckRear.next = j1.next;
			j1.next = temp;
			deckRear = j1;
			return;
		}
	}
	
	/**
	 * Implements Step 4 - Count Cut - on the deck.
	 */
	void countCut() {		
		int count = deckRear.cardValue;
		if (count == 28){
			count = 27;
		}
		CardNode ptr = deckRear;
		for (int i = 0; i < count; i++){
			ptr = ptr.next;
		}
		CardNode beforeRear = deckRear.next;
		while (beforeRear.next != deckRear){
			beforeRear = beforeRear.next;
		}
		beforeRear.next = deckRear.next;
		deckRear.next = ptr.next;
		ptr.next = deckRear;
	}
	
	/**
	 * Gets a key. Calls the four steps - Joker A, Joker B, Triple Cut, Count Cut, then
	 * counts down based on the value of the first card and extracts the next card value 
	 * as key. But if that value is 27 or 28, repeats the whole process (Joker A through Count Cut)
	 * on the latest (current) deck, until a value less than or equal to 26 is found, which is then returned.
	 * 
	 * @return Key between 1 and 26
	 */
	int getKey() {
		while (true){
			
			jokerA();
			
			jokerB();
			
			tripleCut();
			
			countCut();
			
			CardNode ptr = deckRear.next;
			int count = ptr.cardValue;
			if (count == 28){
				count = 27;
			}
			for (int i = 0; i < count; i++){
				ptr = ptr.next;
			}
			if (ptr.cardValue >= 27){
				continue;
			}
			else {
				return ptr.cardValue;
			}
		}
		
	}
	
	/**
	 * Utility method that prints a circular linked list, given its rear pointer
	 * 
	 * @param rear Rear pointer
	 */
	private static void printList(CardNode rear) {
		if (rear == null) { 
			return;
		}
		System.out.print(rear.next.cardValue);
		CardNode ptr = rear.next;
		do {
			ptr = ptr.next;
			System.out.print("," + ptr.cardValue);
		} while (ptr != rear);
		System.out.println("\n");
	}

	/**
	 * Encrypts a message, ignores all characters except upper case letters
	 * 
	 * @param message Message to be encrypted
	 * @return Encrypted message, a sequence of upper case letters only
	 */
	public String encrypt(String message) {	
		String fixed = "";
		for (int i = 0; i < message.length(); i++){
			if (!Character.isLetter(message.charAt(i))){
				continue;
			}
			else {
				fixed += message.charAt(i);
			}
		}
		String encrypted = "";
		
		for (int i = 0; i < fixed.length(); i++){
			int key = getKey();
			
			char c = fixed.charAt(i);
			int ch = c - 'A' + 1;
			int sum = key + ch;
			if (sum > 26){
				sum = sum - 26;
			}
			c = (char)(sum - 1 + 'A');
			encrypted += c;
		}
		return encrypted;
	}
	
	/**
	 * Decrypts a message, which consists of upper case letters only
	 * 
	 * @param message Message to be decrypted
	 * @return Decrypted message, a sequence of upper case letters only
	 */
	public String decrypt(String message) {	
		String fixed = "";
		for (int i = 0; i < message.length(); i++){
			if (!Character.isLetter(message.charAt(i))){
				continue;
			}
			else {
				fixed += message.charAt(i);
			}
		}
		
		String decrypted = "";
		
		for (int i = 0; i < fixed.length(); i++){
			int key = getKey();
			
			char c = fixed.charAt(i);
			int number = c - 'A' + 1;
			if (number <= key){
				number = number + 26;
			}
			int difference = number - key;
			c = (char)(difference - 1 + 'A');
			decrypted += c;
		}
		return decrypted;
		
	}
}
